...
class Test (models.Model):
    ...
    description_1 = models.CharField('Описание до', max_length=255)
    description_2 = models.CharField('Описание после', max_length=255)
    comment = models.CharField('Комментарий для преподавателя', max_length=255)
    ...
    status = models.CharField('Статус теста', max_length=255)
...
class Subtest (models.Model):
    ...
    queue = models.IntegerField('Порядок следования субтеста')
    description_1 = models.CharField('Описание до', max_length=255)
    description_2 = models.CharField('Описание после', max_length=255)
    comment = models.CharField('Комментарий для преподавателя', max_length=255)
    time_for_solution = models.BooleanField('Записывать время прохождения?')
...
class Questions (models.Model):
    ...
    queue = models.IntegerField('Порядок следования вопроса')
    type_question = models.CharField('Тип вопроса', max_length=255)
    obligatory = models.BooleanField('Обязательный ?')
    mix_question = models.BooleanField('Перемешивать вопросы?')
    status = models.CharField('Статус вопроса', max_length=255)
...
class Answers (models.Model):
    name = models.CharField('Название ответа', max_length=255)
    queue = models.IntegerField('Порядок следования ответа')
    description = models.CharField('Описание ответа', max_length=255)
    score = models.IntegerField('Количество баллов')
    right = models.BooleanField('Ответ верный?')
    status = models.CharField('Статус ответа', max_length=255)
...
class Scales (models.Model):
    name = models.CharField('Название шкалы', max_length=255)
    queue = models.IntegerField('Порядок следования шкалы')
    description = models.CharField('Описание шкалы', max_length=255)
    status = models.CharField('Статус шкалы', max_length=255)
...
class Interpretations (models.Model):
    ...
    queue = models.IntegerField('Порядок следования интерпретации')
...
class Attemption (models.Model):
    ...
    date = models.DateTimeField('Дата и время прохождения', auto_now=True)
    time_spent = models.IntegerField ('Затраченное время')
